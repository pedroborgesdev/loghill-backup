...
export function SenderAssociationsDialog({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      await waitForMinimumLoading(startedAt);
      setError(requestError instanceof Error ? requestError.message : "Unable to load associations.");
      setLoadedKind(kind);
    } finally {
      setLoading(false);
    }
  ...
  const label = isAlerts ? "Alerts" : "Events";
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Unable to save associations.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[220] grid place-items-center p-3 sm:p-5">
      <button type="button" aria-label="Close associations" className="absolute inset-0 bg-black/75" onClick={() => !saving && onClose()} />
      <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} className="relative flex max-h-[85dvh] w-full max-w-xl flex-col overflow-hidden rounded-xl border border-zinc-700 bg-[#161618] shadow-2xl shadow-black/70 outline-none">
        <header className="flex items-start justify-between gap-3 border-b border-zinc-800 p-5">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500">Select to associate and clear to remove this sender.</p>
          </div>
          <ModalCloseButton label={`Close sender ${kind === "alerts" ? "alerts" : "events"}`} disabled={saving} onClick={onClose} />
        </header>
          ...
          {loading || loadedKind !== kind ? <div className="space-y-2">{Array.from({ length: 5 }, (_, index) => <Skeleton key={index} className="h-14 w-full" />)}</div> : items.length ? (
            <div className="space-y-1.5">{items.map((item) => {
              ...
              return <button key={item.id} type="button" role="checkbox" aria-checked={checked} disabled={saving || locked} onClick={() => toggle(item)} className="flex w-full items-center gap-3 rounded-lg border border-zinc-800 bg-[#1c1c1f] px-3 py-2.5 text-left transition-colors hover:border-zinc-600 disabled:cursor-not-allowed disabled:opacity-55">
                ...
                <span className={`text-[10px] ${item.enabled ? "text-emerald-500" : "text-zinc-600"}`}>{item.enabled ? "Active" : "Inactive"}</span>
              </button>;
            })}</div>
          ) : <div className="grid min-h-40 place-items-center text-center"><div><p className="text-sm text-zinc-300">No {isAlerts ? "alert" : "event"} configured</p><p className="mt-1 text-xs text-zinc-600">Create one in the {label.toLowerCase()} section to associate it here.</p></div></div>}
        </div>
        <footer className="flex items-center justify-between gap-3 border-t border-zinc-800 bg-zinc-950 px-5 py-3">
          <span className="text-[10px] text-zinc-600">{changed.length} change{changed.length === 1 ? "" : "s"}</span>
          <div className="flex gap-2"><Button onClick={onClose} disabled={saving}>Cancel</Button><Button onClick={() => void save()} disabled={saving || loading || !changed.length}>{saving && <RefreshCw className="size-4 animate-spin" />}{saving ? "Saving..." : "Save associations"}</Button></div>
        </footer>
      </div>
    </div>,
    document.body,
