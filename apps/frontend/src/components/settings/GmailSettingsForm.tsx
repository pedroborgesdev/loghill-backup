...
export function GmailSettingsForm({ value, passwordConfigured, managed, disabled, onChange }: { value: GmailDraft; passwordConfigured: boolean; managed: boolean; disabled: boolean; onChange: (value: GmailDraft) => void }) {
  ...
  return <div className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-950/55 p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><Field label="Sender email"><Input disabled={locked} type="email" value={value.from} onChange={(e) => update("from", e.target.value)} placeholder="your-email@gmail.com" className="mt-1.5 w-full" /></Field><Field label="Sender name"><Input disabled={locked} maxLength={100} value={value.sender_name} onChange={(e) => update("sender_name", e.target.value)} placeholder="LogMate" className="mt-1.5 w-full" /></Field></div>
  </div>;
}
