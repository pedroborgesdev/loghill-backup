...
function Actions({ alert, busy, props }: { alert: EmailAlert; busy: boolean; props: Props }) {
  return <div className="flex justify-end gap-0.5"><IconButton label="View details" className="size-7" disabled={busy} onClick={() => props.onDetails(alert)}><Eye className="size-3.5" /></IconButton><IconButton label="Edit alert" className="size-7" disabled={busy} onClick={() => props.onEdit(alert)}><Edit3 className="size-3.5" /></IconButton><IconButton label={alert.enabled ? "Disable alert" : "Enable alert"} className="size-7" disabled={busy} onClick={() => props.onToggle(alert)}><Power className="size-3.5" /></IconButton><IconButton label="Send test" className="size-7" disabled={busy} onClick={() => props.onTest(alert)}><Send className="size-3.5" /></IconButton><IconButton label="Delete alert" className="size-7 text-red-400" disabled={busy} onClick={() => props.onDelete(alert)}><Trash2 className="size-3.5" /></IconButton></div>;
}
