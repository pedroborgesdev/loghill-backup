...
4. Adicione uma ou mais ações de evento, e-mail ou requisição HTTP.
...
Tipos iniciais de condição: evento, alerta, mensagem, severity, metadata, horário, dia da semana e data. As ações podem disparar um evento existente, enviar e-mail ou enfileirar uma requisição HTTP com método, URL, headers, cookies e body configuráveis.

A ação HTTP exige uma URL HTTPS pública e reutiliza a proteção contra SSRF e a outbox dos eventos. O fluxo da regra não aguarda a resposta remota; o worker fecha a resposta após os cabeçalhos e não baixa o body. Headers e cookies ficam persistidos na regra e devem usar credenciais de escopo mínimo.
