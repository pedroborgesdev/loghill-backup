...
func newEventHTTPFixture(t *testing.T, admin bool) eventHTTPFixture {
	...
	controller := controllers.New(svc, cfg, sched, assets).ConfigureNotifications(alertService, notificationService).ConfigureEvents(eventService)
	return eventHTTPFixture{router: New(controller, cfg).Router(), sender: sender, senderKey: credentials.SenderKey, events: eventService, dispatcher: dispatcher}
}
