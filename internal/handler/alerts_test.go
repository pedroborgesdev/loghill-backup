...
func newAlertHTTPFixture(t *testing.T, admin bool) alertHTTPFixture {
	...
	api := New(svc, cfg, sched, assets).ConfigureNotifications(alertService, emailSettings, provider, dispatcher)
	return alertHTTPFixture{router: api.Router(), sender: sender, dispatcher: dispatcher}
}
