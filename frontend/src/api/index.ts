...
import type { CreateSenderRequest, CreateSenderResponse, HealthResponse, LogPage, ReactivateSenderResponse, RotateSenderKeyResponse, Sender, SenderDependencies, SenderPage, Settings, SettingsUpdateResponse, Summary } from "../types/api";
export const api={
  ...
  deleteSender:(id:string,removeFromAlerts=false,removeFromEvents=false)=>{const query=new URLSearchParams();if(removeFromAlerts)query.set("remove_from_alerts","true");if(removeFromEvents)query.set("remove_from_events","true");return request<void>(`/api/v1/senders/${encodeURIComponent(id)}${query.size?`?${query}`:""}`,{method:"DELETE"})},
