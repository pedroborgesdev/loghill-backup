...
import { Button, ConfirmDialog, ErrorAlert, MetricCard, Pagination, Panel, SearchInput } from "../components/ui";
...
export function AlertsPage() {
  ...
  const [data, setData] = useState<AlertPage>();
  const dataRef = useRef<AlertPage | undefined>(undefined);
  ...
  }, [debounced, page, pageSize, setRefreshing]);
  ...
  return (
    <div className="space-y-4">
      ...
      {!outlookReady && data && <div className="flex flex-wrap items-center gap-3 rounded-xl border border-amber-950 bg-amber-950/15 px-4 py-3"><AlertTriangle className="size-4 text-amber-500" /><div className="min-w-0 flex-1"><p className="text-xs font-medium text-amber-300">Outlook não configurado</p><p className="text-[11px] text-zinc-500">Configure e habilite o provedor para ativar regras e enviar testes.</p></div><Button onClick={(event) => openEmailSettings(event.currentTarget)} className="h-8">Configurar Outlook</Button></div>}
      {outlookReady && <div className="flex items-center gap-2 rounded-lg border border-emerald-950 bg-emerald-950/15 px-3 py-2 text-xs text-emerald-400"><MailCheck className="size-4" />Outlook configurado{data?.email_provider.last_test_status === "success" ? " e validado" : ""}</div>}
      ...
      <Panel className="overflow-hidden"><div className="flex min-h-14 flex-col gap-2 border-b border-zinc-800 p-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="text-sm font-medium text-zinc-200">Alertas configurados</h3>{loading && data && <RefreshCw className="size-3 animate-spin text-zinc-600" />}</div><p className="text-[11px] text-zinc-600">Um e-mail é enviado para cada log que corresponder à regra.</p></div><SearchInput value={search} onChange={setSearch} placeholder="Buscar alerta, sender ou destinatário" className="w-full sm:w-80" /><Button onClick={() => void load()} disabled={loading}><RefreshCw className={`size-4 ${loading ? "animate-spin" : ""}`} />Atualizar</Button></div>
        ...
        {initialLoading && !data ? <div className="grid min-h-60 place-items-center text-xs text-zinc-600">Carregando alertas...</div> : data?.items.length ? <div className={loading ? "opacity-70 transition-opacity" : ""}><AlertTable items={data.items} busyId={busyId} onDetails={setDetails} onEdit={setEditing} onToggle={(alert) => void toggle(alert)} onTest={(alert) => void test(alert)} onDelete={setDeleting} /></div> : <div className="grid min-h-64 place-items-center p-6 text-center"><div><Bell className="mx-auto size-7 text-zinc-700" /><p className="mt-3 text-sm font-medium text-zinc-300">Nenhum alerta configurado</p><p className="mt-1 max-w-sm text-xs leading-5 text-zinc-600">Crie um alerta para receber por e-mail os erros enviados pelos seus senders.</p><Button onClick={() => setEditing("new")} className="mt-4"><Plus className="size-4" />Novo alerta</Button></div></div>}
