...
export interface Sender {
  ...
  log_line_count:number; log_file_size:number; recent_error_count?:number;
}
...
export interface SenderDependencies { sender_id:string; alert_rules:number; events:number }
export interface LogEntry { timestamp:string; sender?:string; severity:LogSeverity; message:string; event?:string; event_occurrence_id?:string; metadata?:Record<string,unknown> }
...
export interface Summary { senders:Record<"total"|"never_connected"|"online"|"inactive"|"expired"|"revoked",number>; logs:Record<"total"|"last_24_hours"|"errors_last_24_hours"|"fatal_last_24_hours",number> }
