...
export function SenderCredentialsDialog({ value, onClose }: { value: CredentialsState; onClose: () => void }) {
  ...
  return (
      <ModalFrame title="Sender criado" description="O sender está pronto para receber conexões pelo nome." onClose={onClose} width="max-w-2xl" footer={<Button onClick={onClose} className="border-zinc-500 bg-zinc-800 text-zinc-100 hover:border-zinc-400 hover:bg-zinc-700">Concluir</Button>}>
        <div className="space-y-4">
          <div className="rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-3 text-xs leading-5 text-emerald-300"><div className="flex gap-2"><CheckCircle2 className="mt-0.5 size-4 shrink-0" /><p>Nenhuma chave precisa ser configurada no cliente. Use o identificador abaixo como <code>sender_name</code>.</p></div></div>
          ...
          <div className="grid gap-2 sm:grid-cols-2"><Button onClick={() => void copyText(environment).then(() => setCopied(true))}><Clipboard className="size-4" />Copiar configuração de ambiente</Button><Button onClick={() => void copyText(pythonExample).then(() => setCopied(true))}><Copy className="size-4" />Copiar exemplo em Python</Button></div>
          {copied && <p role="status" className="text-[10px] text-emerald-400">Configuração copiada.</p>}
        </div>
      </ModalFrame>
  );
}
...
export function SenderActionDialogs({ sender, action, onClose, onChanged, onDeleted }: { sender?: Sender; action?: SenderAction; onClose: () => void; onChanged: (sender: Sender) => void; onDeleted: (id: string) => void }) {
  ...
  const titles = { edit: "Editar informações", revoke: "Revogar acesso", reactivate: "Reativar sender", delete: "Excluir sender" };
  const labels = { edit: "Salvar alterações", revoke: "Revogar acesso", reactivate: "Reativar sender", delete: "Excluir sender" };
  return (
    <ModalFrame title={titles[action]} description={action === "edit" ? "O identificador não pode ser alterado após a criação." : undefined} onClose={onClose} closeDisabled={busy} width="max-w-lg" footer={<><Button onClick={onClose} disabled={busy}>Cancelar</Button><Button onClick={() => void execute()} disabled={busy || (action === "edit" && name.trim().length < 3)} className={action === "delete" || action === "revoke" ? "border-red-900 text-red-300 hover:bg-red-950" : "border-zinc-500 bg-zinc-800 text-zinc-100 hover:border-zinc-400 hover:bg-zinc-700"}>{action === "reactivate" && <KeyRound className="size-4" />}{action === "revoke" && <ShieldOff className="size-4" />}{action === "delete" && <Trash2 className="size-4" />}{busy ? "Processando..." : labels[action]}</Button></>}>
      {action === "edit" ? <div className="space-y-4"><label className="block text-xs text-zinc-300">Identificador<Input value={sender.id} readOnly aria-readonly className="mt-2 w-full cursor-not-allowed text-zinc-500" /></label><label className="block text-xs text-zinc-300">Nome de exibição<Input value={name} minLength={3} maxLength={80} onChange={(event) => setName(event.target.value)} className="mt-2 w-full" /><span className="mt-1.5 block text-[10px] text-zinc-600">Mínimo de 3 caracteres.</span></label><label className="block text-xs text-zinc-300">Descrição<textarea value={description} maxLength={250} onChange={(event) => setDescription(event.target.value)} className={`mt-2 h-24 w-full resize-none rounded-lg border p-3 text-sm text-zinc-100 ${CONTROL_SURFACE}`} /></label></div> : <div className="space-y-3 text-sm leading-6 text-zinc-400"><p>{action === "revoke" && "Novas conexões serão bloqueadas e as credenciais das instâncias atuais deixarão de aceitar logs e healthchecks. Os dados e regras existentes serão preservados."}{action === "reactivate" && "O sender voltará a aceitar conexões pelo sender_name. Nenhuma chave precisa ser configurada nos clientes."}{action === "delete" && dependencies && (dependencies.alert_rules || dependencies.events || dependencies.monitoring_rules) ? `Este sender está associado a ${dependencies.alert_rules} regra(s) de alerta, ${dependencies.events} evento(s) e ${dependencies.monitoring_rules} regra(s) de monitoramento. Ao continuar, ele será removido dessas configurações; as que ficarem sem senders serão desativadas.` : action === "delete" ? "Os logs e os dados deste sender serão excluídos permanentemente." : ""}</p><code className="block rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-300">{sender.id}</code></div>}
