...
export function SenderMultiSelect({ value, onChange, disabled = false, required = false }: { value: SenderOption[]; onChange: (senders: SenderOption[]) => void; disabled?: boolean; required?: boolean }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between"><label className="text-xs font-medium text-zinc-300">Senders <span className="font-normal text-zinc-600">({required ? "obrigatório" : "opcional"})</span></label><span className="text-[10px] tabular-nums text-zinc-500">{value.length} selecionado{value.length === 1 ? "" : "s"}</span></div>
      ...
      <div className="mt-2 overflow-hidden rounded-lg border border-zinc-700 bg-[#1c1c1f] transition-colors duration-150 hover:border-zinc-600 focus-within:border-sky-400/70">
        <label className="relative block border-b border-zinc-800"><span className="sr-only">Buscar senders</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-600" /><input disabled={disabled} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar sender pelo nome" className="h-9 w-full bg-transparent pl-9 pr-3 text-xs text-zinc-200 outline-none placeholder:text-zinc-700" />{loading && items.length > 0 && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-zinc-600">Atualizando...</span>}</label>
