...
export function AlertFormDialog({ alert, outlookReady, onSaved, onClose, onConfigureOutlook }: Props) {
  ...
  const validation = useMemo(() => {
    if (name.trim().length < 3 || name.trim().length > 100) return "The name must be between 3 and 100 characters.";
    if (!severities.length) return "Select at least one severity.";
    if (!recipients.length) return "Add at least one recipient.";
    if (enabled && !outlookReady) return "Configure and enable email or save the alert as inactive.";
    return "";
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Unable to save the alert.");
  ...
  return createPortal(
    <div className="fixed inset-0 z-[210] grid place-items-center p-3 sm:p-5">
      ...
      <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} className="relative flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-zinc-700 bg-[#111113] shadow-2xl shadow-black/70 outline-none">
        <header className="flex shrink-0 items-start justify-between border-b border-zinc-800 px-5 py-4"><div><h2 id={titleId} className="text-base font-semibold text-zinc-100">{alert ? "Edit alert" : "New email alert"}</h2><p className="mt-1 text-xs text-zinc-500">Each matching log creates an individual notification.</p></div><ModalCloseButton label="Close alert form" disabled={saving} onClick={onClose} /></header>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          {!outlookReady && <div className="flex items-start gap-3 rounded-lg border border-amber-950 bg-amber-950/20 p-3"><AlertTriangle className="mt-0.5 size-4 shrink-0 text-amber-500" /><div className="text-xs leading-5 text-amber-300"><p>No email provider is configured and enabled.</p><button type="button" onClick={(event) => onConfigureOutlook(event.currentTarget)} className="mt-1 font-medium underline underline-offset-2">Configure email</button></div></div>}
          <label className="block text-xs font-medium text-zinc-300">Alert name<Input autoFocus disabled={saving} value={name} minLength={3} maxLength={100} onChange={(event) => { setName(event.target.value); setError(""); }} placeholder="Critical errors in financial automation" aria-invalid={field === "name"} className="mt-2 w-full" /><span className="mt-1.5 block text-[10px] text-zinc-600">Minimum of 3 characters.</span></label>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg border border-zinc-700 bg-zinc-900 text-sm">@</div><div><p className="text-sm font-medium text-zinc-200">Email</p><p className="text-[11px] text-zinc-600">Global provider selected</p></div><span className={`ml-auto text-[11px] ${outlookReady ? "text-emerald-400" : "text-amber-400"}`}>{outlookReady ? "Configured" : "Not configured"}</span></div></div>
          <button type="button" role="switch" aria-checked={enabled} disabled={saving} onClick={() => setEnabled((value) => !value)} className="flex w-full items-center justify-between rounded-xl border border-zinc-800 bg-zinc-950/60 p-4 text-left focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/50"><span><span className="block text-sm font-medium text-zinc-200">Active alert</span><span className="mt-1 block text-[11px] text-zinc-600">Inactive rules remain saved without sending messages.</span></span><span className={`relative h-6 w-11 rounded-full transition-colors ${enabled ? "bg-emerald-600" : "bg-zinc-700"}`}><span className={`absolute top-1 size-4 rounded-full bg-white transition-transform ${enabled ? "translate-x-6" : "translate-x-1"}`} /></span></button>
          {error && <p role="alert" className="rounded-lg border border-red-950 bg-red-950/20 px-3 py-2 text-xs text-red-300">{error}</p>}
        </div>
        <footer className="flex shrink-0 justify-end gap-2 border-t border-zinc-800 bg-zinc-950 px-5 py-3"><Button onClick={onClose} disabled={saving} className="border-transparent bg-transparent">Cancel</Button><Button onClick={() => void save()} disabled={saving || Boolean(validation)} className="border-zinc-500 bg-zinc-800 text-zinc-100 hover:border-zinc-400 hover:bg-zinc-700"><Check className="size-4" />{saving ? "Saving..." : "Save alert"}</Button></footer>
      </div>
    </div>, document.body,
  );
}
