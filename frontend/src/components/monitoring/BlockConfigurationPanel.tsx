...
const operators: Record<string, { value: string; label: string }[]> = {
  message: [{ value: "contains", label: "contains" }, { value: "not_contains", label: "does not contain" }, { value: "equals", label: "equals" }, { value: "not_equals", label: "not equals" }, { value: "starts_with", label: "starts with" }, { value: "ends_with", label: "ends with" }],
  ...
  return <aside className="absolute inset-y-0 right-0 z-20 flex w-[min(88vw,22rem)] flex-col border-l border-zinc-700 bg-[#111113] shadow-2xl lg:static lg:w-80 lg:min-w-80 lg:shadow-none">
    ...
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
      ...
      {block.condition && block.type !== "log_received" && <>
        ...
        {block.type === "message" && <Field label="Text"><Input value={String(block.condition.value.text ?? "")} onChange={(event) => setValue("text", event.target.value)} placeholder="ECONNRESET" className="w-full" /></Field>}
