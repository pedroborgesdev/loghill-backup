...
export function OutlookSettingsForm({ value, secretConfigured, managed, disabled, onChange }: { value: OutlookDraft; secretConfigured: boolean; managed: boolean; disabled: boolean; onChange: (value: OutlookDraft) => void }) {
  ...
  return (
    <div className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-950/55 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2"><Field label="Sender email"><Input disabled={locked} type="email" value={value.sender_email} onChange={(event) => update("sender_email", event.target.value)} placeholder="logs@company.com" className="mt-1.5 w-full" /></Field><Field label="Sender name"><Input disabled={locked} maxLength={100} value={value.sender_name} onChange={(event) => update("sender_name", event.target.value)} placeholder="LogMate" className="mt-1.5 w-full" /></Field></div>
    </div>
  );
}
