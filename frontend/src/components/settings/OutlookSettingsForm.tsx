...
export function OutlookSettingsForm({ value, secretConfigured, managed, disabled, onChange }: { value: OutlookDraft; secretConfigured: boolean; managed: boolean; disabled: boolean; onChange: (value: OutlookDraft) => void }) {
  ...
  return (
    <div className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-950/55 p-4">
      {managed && <div className="flex items-center gap-2 rounded-lg border border-blue-950 bg-blue-950/20 px-3 py-2 text-[11px] text-blue-300"><KeyRound className="size-3.5" />Configuração gerenciada por variáveis de ambiente. Os campos ficam bloqueados, mas os testes permanecem disponíveis.</div>}
      <button type="button" role="switch" aria-checked={value.enabled} disabled={locked} onClick={() => update("enabled", !value.enabled)} className="flex w-full items-center justify-between rounded-lg text-left focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/50"><span><span className="block text-xs font-medium text-zinc-200">Habilitar envio por Outlook</span><span className="mt-1 block text-[10px] text-zinc-600">Necessário para ativar novos alertas.</span></span><span className={`relative h-6 w-11 rounded-full ${value.enabled ? "bg-emerald-600" : "bg-zinc-700"}`}><span className={`absolute top-1 size-4 rounded-full bg-white transition-transform ${value.enabled ? "translate-x-6" : "translate-x-1"}`} /></span></button>
      ...
      <Field label="Client Secret"><div className="relative mt-1.5"><Input disabled={locked} type={showSecret ? "text" : "password"} autoComplete="new-password" value={value.client_secret} onChange={(event) => update("client_secret", event.target.value)} placeholder={secretConfigured ? "Credencial configurada — digite apenas para substituir" : "Informe a credencial"} className="w-full pr-10 font-mono text-xs" /><button type="button" disabled={locked} aria-label={showSecret ? "Ocultar credencial" : "Mostrar credencial digitada"} onClick={() => setShowSecret((current) => !current)} className="absolute right-1 top-1 grid size-7 place-items-center rounded text-zinc-600 hover:bg-zinc-800 hover:text-zinc-300 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/50">{showSecret ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}</button></div>{secretConfigured && !value.client_secret && <span className="mt-1 block text-[10px] text-emerald-500">Credencial configurada. O valor salvo nunca é exibido.</span>}</Field>
      <div className="grid gap-3 sm:grid-cols-2"><Field label="E-mail remetente"><Input disabled={locked} type="email" value={value.sender_email} onChange={(event) => update("sender_email", event.target.value)} placeholder="logs@empresa.com" className="mt-1.5 w-full" /></Field><Field label="Nome do remetente"><Input disabled={locked} maxLength={100} value={value.sender_name} onChange={(event) => update("sender_name", event.target.value)} placeholder="LogHill" className="mt-1.5 w-full" /></Field></div>
    </div>
  );
}
