...
export function EventFormDialog({ event, outlookReady, onSaved, onClose, onConfigureOutlook }: { event?: EventDefinition; outlookReady: boolean; onSaved: (event: EventDefinition) => void; onClose: () => void; onConfigureOutlook: (trigger: HTMLButtonElement) => void }) {
  ...
  const [phoneNumbers, setPhoneNumbers] = useState(event?.phone_numbers ?? []);
  const [smsTemplate, setSMSTemplate] = useState(event?.sms_template ?? "Event {{event.name}} on {{sender.name}}: {{log.message}}");
  ...
  const smsEnabled = actionType === "sms";
  ...
  const errors = useMemo(() => [
    ...
    emailEnabled && !recipients.length ? "Add at least one recipient." : emailEnabled && enabled && !outlookReady ? "Configure email or disable email delivery." : webhookEnabled && !validWebhookURL(webhookURL) ? "Enter a public HTTPS webhook URL without credentials." : smsEnabled && (!phoneNumbers.length || phoneNumbers.some((number) => !/^\+[1-9]\d{7,14}$/.test(number))) ? "Enter at least one valid E.164 phone number." : "",
    emailEnabled && !subject.trim() ? "Enter the email subject." : emailEnabled && (subject.length > 200 || /[\r\n]/.test(subject)) ? "The subject must be at most 200 characters and cannot contain line breaks." : emailEnabled && !message.trim() ? "Enter the event message." : emailEnabled && message.length > 10000 ? "The message must be at most 10,000 characters." : smsEnabled && (!smsTemplate.trim() || smsTemplate.length > 1600) ? "The SMS message is required and must be at most 1,600 characters." : "",
    "",
  ], [emailEnabled, enabled, event, eventKey, keyAvailable, message, name, outlookReady, phoneNumbers, recipients.length, senders.length, smsEnabled, smsTemplate, subject, webhookEnabled, webhookURL]);
  ...
  const save = async () => {
    ...
    const input: EventInput = {
      ...
      phone_numbers: smsEnabled ? phoneNumbers : [], sms_template: smsEnabled ? smsTemplate.trim() : "", enabled,
    };
  ...
  const actionName = actionType === "email" ? "Send email" : actionType === "webhook" ? "Call webhook" : actionType === "sms" ? "Send SMS" : "Monitoring only";
    ...
    <div className="fixed inset-0 z-[210] grid place-items-center p-3 sm:p-5">
      ...
      <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} className="relative flex max-h-[94dvh] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-zinc-700 bg-[#111113] shadow-2xl shadow-black/70 outline-none">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {step === 2 && <div className="mx-auto max-w-2xl space-y-5"><EventActionSelector value={actionType} disabled={saving} onChange={setAction} />{emailEnabled && <EventEmailSettings recipients={recipients} onChange={(value) => { setRecipients(value); setError(""); }} outlookReady={outlookReady} disabled={saving} onConfigureOutlook={onConfigureOutlook} />}{webhookEnabled && <label className="block text-xs font-medium text-zinc-300">Webhook URL<Input value={webhookURL} disabled={saving} aria-invalid={field === "webhook_url" || Boolean(webhookURL && !validWebhookURL(webhookURL))} onChange={(change) => { setWebhookURL(change.target.value); setError(""); }} placeholder="https://hooks.example.com/loghill" className="mt-2 w-full font-mono text-xs" /><span className="mt-2 flex items-start gap-2 text-[11px] leading-4 text-zinc-500"><ShieldCheck className="mt-0.5 size-3.5 shrink-0 text-emerald-500" />Only public HTTPS destinations are accepted. Redirects and private networks are blocked.</span></label>}{smsEnabled && <label className="block text-xs font-medium text-zinc-300">Phone numbers<Input value={phoneNumbers.join(", ")} disabled={saving} aria-invalid={field === "phone_numbers"} onChange={(change) => { setPhoneNumbers(change.target.value.split(",").map((value) => value.trim()).filter(Boolean)); setError(""); }} placeholder="+5511999999999, +15551234567" className="mt-2 w-full font-mono text-xs" /><span className="mt-2 block text-[11px] text-zinc-500">Comma-separated E.164 numbers. Twilio must be enabled on the server.</span></label>}</div>}
          {step === 3 && (emailEnabled ? <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,.8fr)]"><EventTemplateEditor subject={subject} message={message} onSubjectChange={(value) => { setSubject(value); setError(""); }} onMessageChange={(value) => { setMessage(value); setError(""); }} disabled={saving} /><EventTemplatePreview name={name} eventKey={eventKey} subject={subject} message={message} /></div> : smsEnabled ? <label className="mx-auto block max-w-2xl text-xs font-medium text-zinc-300">SMS message<textarea value={smsTemplate} disabled={saving} maxLength={1600} aria-invalid={field === "sms_template"} onChange={(change) => { setSMSTemplate(change.target.value); setError(""); }} className="mt-2 min-h-40 w-full rounded-lg border border-zinc-700 bg-zinc-900 p-3 text-sm outline-none" /><span className="mt-2 block text-[11px] text-zinc-500">{smsTemplate.length}/1600. The same event template variables are supported.</span></label> : <div className="mx-auto max-w-2xl rounded-xl border border-zinc-800 bg-zinc-950/50 p-5 text-sm text-zinc-400">{webhookEnabled ? "The webhook receives a structured JSON payload; no text template is required." : "No template is required. This event will only be used as a monitoring trigger."}</div>)}
          {step === 4 && <div className="mx-auto max-w-2xl space-y-4"><section className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-4"><h3 className="text-sm font-medium text-zinc-200">Review before saving</h3><dl className="mt-4 grid gap-4 text-xs sm:grid-cols-2"><Review label="Name" value={name} /><Review label="Key" value={eventKey} mono /><Review label="Senders" value={`${senders.length} selected`} /><Review label="Action" value={actionName} /><Review label="Recipients" value={emailEnabled ? `${recipients.length} address${recipients.length === 1 ? "" : "es"}` : smsEnabled ? `${phoneNumbers.length} phone number${phoneNumbers.length === 1 ? "" : "s"}` : "Not applicable"} /><Review label="Destination" value={webhookEnabled ? webhookURL : emailEnabled ? (outlookReady ? "Email configured" : "Email unavailable") : smsEnabled ? "Twilio SMS" : "Internal only"} mono={webhookEnabled} /></dl></section><button type="button" role="switch" aria-checked={enabled} disabled={saving} onClick={() => { setEnabled((value) => !value); setError(""); }} className="flex w-full items-center justify-between rounded-xl border border-zinc-800 bg-zinc-950/60 p-4 text-left"><span><span className="block text-sm font-medium text-zinc-200">Active event</span><span className="mt-1 block text-[11px] text-zinc-600">Inactive events remain saved and do not react when the key is received.</span></span><span className={`relative h-6 w-11 rounded-full ${enabled ? "bg-emerald-600" : "bg-zinc-700"}`}><span className={`absolute top-1 size-4 rounded-full bg-white transition-transform ${enabled ? "translate-x-6" : "translate-x-1"}`} /></span></button>{emailEnabled && !outlookReady && enabled && <p className="text-xs text-amber-400">Configure email or disable the delivery action to save the active event.</p>}</div>}
