...
export function EventKeyField({ value, onChange, immutable = false, disabled = false, onAvailabilityChange }: { value: string; onChange: (value: string) => void; immutable?: boolean; disabled?: boolean; onAvailabilityChange?: (available: boolean) => void }) {
  ...
  const feedback = immutable ? "A chave é imutável depois da criação." : !value ? "Use letras minúsculas, números, underscore ou hífen." : !valid ? "A chave deve ter entre 3 e 80 caracteres e não pode conter espaços ou acentos." : checking ? "Verificando disponibilidade..." : available ? "Chave disponível." : available === false ? "Esta chave já está em uso." : "Não foi possível verificar a chave.";
  return (
    <div>
      <div className="flex items-center justify-between"><label htmlFor="event-key" className="text-xs font-medium text-zinc-300">Chave do evento</label><button type="button" disabled={!value} onClick={() => void navigator.clipboard.writeText(value)} className="inline-flex items-center gap-1 rounded px-1.5 py-1 text-[10px] text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/50 disabled:opacity-40"><Clipboard className="size-3" />Copiar chave</button></div>
